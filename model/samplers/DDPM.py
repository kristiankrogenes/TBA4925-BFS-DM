...
class DDPMSampler(nn.Module):
    ...
    def posterior_params(self,x_t,t,noise_pred):
        ...
        alphas_sqrt_recip = self.alphas_sqrt_recip.to(self.device)
        betas_sqrt = self.betas_sqrt.to(self.device)

        beta_t = betas[t].view(x_t.shape[0], 1, 1, 1)
        alpha_one_minus_cumprod_sqrt_t = alphas_one_minus_cumprod_sqrt[t].view(x_t.shape[0], 1, 1, 1)
        alpha_sqrt_recip_t = alphas_sqrt_recip[t].view(x_t.shape[0], 1, 1, 1)
        
        mean = alpha_sqrt_recip_t * (x_t-beta_t * noise_pred / alpha_one_minus_cumprod_sqrt_t)
        std = betas_sqrt[t].view(x_t.shape[0], 1, 1, 1)
